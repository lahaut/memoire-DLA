...
Les illustrations accompagnant ce document proviennent de simulation réalisées en OCaml et Python.

\begin{center}
 \includegraphics[scale=0.33]{100000.png}
\end{center}
...
\ \mathrm{et}\ \limsup\limits_{n\to\infty}\frac{\delta_O(n)}{\sqrt{\log(n)}}\leqslant\beta_d .  
  \end{equation}
  En dimension 2, $\exists\beta_2>0$ tel que
   \begin{equation}
\limsup\limits_{n\to\infty}\frac{\delta_I(n)}{\log(n)}\leqslant\beta_2
\ \mathrm{et}\ \limsup\limits_{n\to\infty}\frac{\delta_O(n)}{\log(n)}\leqslant\beta_2 . 
  \end{equation}
 \end{The}
 ...
 \begin{Rq}
  Dans \cite{ref2}, il est également prouvé par des méthodes similaires qu'en dimension $d=2$, $\exists\alpha_2 > 0$ tel que
  \begin{equation}
   \lim_{n\to\infty}P(\exists k\geqslant n,\ \delta_0(k)\geqslant h(k)\ \normalfont{|}\ \delta_I(n)<h(n))=1
  \end{equation}
  où $h(n)=\alpha_2\sqrt{\log(n)\log(\log(n))}$.
 \end{Rq}
Autrement dit, si les trous ne sont pas trop grand, alors de longs tentacules apparaissent. Ce théorème montre que l'erreur externe croît comme on s'y attendait sous l'hypothèse d'une erreur interne faible, mais il ne permet malheureusement pas de conclure. Ce résultat est tout de même intéressant comme le montre le corollaire suivant :
\begin{Cor} \label{cio}
 \begin{equation} \label{io}
   P(\limsup\limits_{n\to\infty}\{\delta_I(n)\geqslant L(n)\}\cup\{\delta_O(n)\geqslant h(n)\})=1.
  \end{equation}
\end{Cor}
Ainsi, pour une infinté de terme, soit l'erreur externe, soit l'erreur interne est supérieure à $\sqrt{log(n)}$.
\medskip

Le cas de l'erreur interne est quant à lui complètement déterminé en dimension supérieure à 3 grâce au résultat suivant, qui dit que presque sûrement de grands trous apparaissent une infinité de fois :
 \begin{The} \label{int}
  En dimension $d \geqslant 3$, $\exists\alpha_d>0$ tel que
  \begin{equation}
 ...
 \label{abel}
 On va prouver ici que le choix de l'ordre de lancer des explorateurs ne change rien à la loi de la région explorée. Le lemme suivant est issu de \cite{ref4} :
 \begin{Lemme}
 ...
 Si on commence par lancer depuis $x$, on obtient l'ensemble $\Lambda \cup \{w\}$ avec probabilité $p(x,\Lambda,w)$ et l'ensemble $\Lambda \cup \{z\}$ avec probabilité $p(x,\Lambda,z)$. On lance ensuite le second explorateur depuis $y$, et on obtient l'ensemble $\Lambda \cup \{w,z \}$ avec probabilité
...
Illustrons la propriété abélienne par l'exemple d'une exploration par vagues, qui nous sera utile par la suite. On va réaliser la région explorée $A(n+m)$ en $3$ vagues. La première vague consiste à faire partir $n$ explorateurs de l'origine et à considérer la région explorée correspondante $A_1=A(n)$ (en bleu sur la figure \ref{vague}). 

Pour la deuxième vague, on fait partir $m$ explorateurs de l'origine avec pour région préalablement explorée $A_1$ et on les arrête sur $\partial \mathbb{B}(0,R)$ (en vert sur \ref{vague}). On note $A_2$ la région explorée après les deux vagues et $\Delta_R$ la configuration des explorateurs de la deuxième vague arrêtés sur $\partial\mathbb{B}(0,R)$ :
\begin{equation*}
 A_2=A_1\cup A_R(A_1,\zeta_0^m), 
...
\begin{enumerate}[i)]
  ...
  \item \label{iii}Si $\eta:\mathbb{Z}^d\to\mathbb{N}$ est telle que $\eta(z)$ est le nombre d'explorateurs partis de $z$, la région explorée en partant de $\eta$ sera la région définie en faisant partir les explorateurs. Par propriété abélienne, l'ordre des lancers n'est pas important ; on peut choisir un ordre prédéfini sur $\mathbb{Z}^d$. 
...
\end{enumerate}
 ...
 \end{Rq}
Pour démontrer ce théorème, procédons de la façon suivante : on va lancer les explorateurs à l'aide d'un processus par vagues comme expliqué dans la section \ref{abel}. Une fois les explorateurs de la deuxième vague arrêtés, on va estimer la probabilité qu'il existe $z\in\partial\mathbb{B}(0,R)$ tel que les explorateurs arrêtés en $z$ sortent de la boule $\mathbb{B}(0,R+h(R))$, et donc que $\delta_O(R)\geqslant h(R)$.
\subsection{Conditions d'apparition d'un tentacule}
 On pose $h(n)=\alpha\sqrt{\log(n)}$ et $L(n)=\gamma\sqrt{\log(n)}$ où $\alpha$ et $\gamma$ sont des constantes à choisir par la suite. Ici, $h$ représente l'erreur externe et $L$ l'erreur interne, et il est important de conserver cette distinction à l'esprit même si on prendra $\alpha=\gamma$ par la suite.
...
\begin{figure}[H]
 \centering
   \includegraphics[scale=0.8]{tentacule.png}
   \caption{La suite des $y_n$}
\label{vagueimage}
\end{figure}
...
\begin{equation*}
 \left( \frac{1}{2d} \right) ^{1+2+\dots+k}\geqslant \exp(-c(R-\|z\|)^2),
\end{equation*}
...
\end{proof}
 ...
 On fixe désormais l'entier $n$, et on va essayer d'estimer la probabilité de trouver un entier $k\geqslant n$ pour lequel l'erreur externe dépasse $h(k)$. Pour cela on va utiliser un processus par vagues comme décrit dans la section \ref{abel} en lançant les explorateurs de l'origine en trois vagues.
 
 La première vague est consituée de $b(n)$ explorateurs partis de l'origine qui explorent $A_1$. La deuxième est constituée $X_n$ explorateurs partis de l'origine, où $X_n$ est une variable aléatoire discrète, indépendante des marches des explorateurs, et dont la loi sera fixée par la suite. Les explorateurs de la deuxième vague sont arrêtés sur $\Sigma=\partial\mathbb{B}(0,r_n)$, avec $r_n = n-L(n)$. La troisième est constituée des explorateurs de la seconde vague relancés.
...
\end{equation}
où $N_z=W_{r_n}(A_1;\zeta_0^{X_n},z)$ est le nombre d'explorateurs arrétés en $z$ et $\Lambda_n$ est la région explorée au moment où on a arrété la deuxième vague. Les $\mathrm{cov}(z)$ ne sont pas indépendants pour le moment.
\medskip

\begin{figure}[H]
 \centering
   \includegraphics[scale=1.1]{cov.png}
   \caption{Un exemple où $\mathrm{cov}(z)$ est réalisé}
\end{figure}
\medskip
\pagebreak[2]

On constate que \{$\delta_O(R_n)\geqslant h(R_n)$\} est lié à $\{\mathrm{cov}(z), z\in\Sigma\}$, car si un $\mathrm{cov}(z)$ est réalisé alors $\delta_O(R_n)\geqslant h(R_n)$, en effet :
\begin{align}
\nonumber P(\delta_O(R_n)\geqslant h(R_n))&\geqslant P(A(b(R_n))\not\subset\mathbb{B}(0,R_n+h(R_n)))
 \nonumber\\&\geqslant P(A(b(n)+X_n)\not\subset\mathbb{B}(0,R_n+h(R_n)))
 \nonumber\\&\geqslant P\left( A\left( \Lambda_n,\sum_{z\in\Sigma}\zeta_z^{N_z} \right) \not\subset\mathbb{B}(0,R_n+h(R_n)) \right)
 \nonumber\\&\geqslant P\left(\bigcup_{z\in\Sigma}\mathrm{cov}(z)\right).
\end{align}

Il nous faut désormais estimer $P(\bigcup_{z\in\Sigma}\mathrm{cov}(z))$. Pour cela, il serait intéressant d'avoir indépendance entre les $\mathrm{cov}(z)$ sous certaines hypothèses, ce qu'on va obtenir en prenant pour $X_n$ une variable de Poisson. En effet, les variables de Poisson ont la propriété de découpage \ref{indé}, qui laisse penser que les $N_z$ seront indépendantes et suivront des lois de Poisson.

Cette propriété ne permet pas pour autant d'affirmer que les $N_z$ sont indépendantes de ce qui se passe avant, mais cela sera vrai lorsque $\delta_I(n) < L(n)$. On définit en effet pour tout $z\in\Sigma$
\begin{equation}
...
\begin{figure}[H]
 \centering
   \includegraphics[scale=0.95]{cov2.png}
...
\begin{align}
\label{17}
\nonumber P(\bigcup_{z\in\Sigma}\mathrm{cov_2}(z))&\geqslant P(\bigcup_{z\in\Sigma}\mathrm{cov_2}(z)\cap\{X_n\leqslant 2 \lambda_n\})
\nonumber\\&\geqslant P(\bigcup_{z\in\Sigma}\{A(\Delta_n,\zeta_z^{N'_z})\not\subset\mathbb{B}(0,n+3h(n))\})P(X_n\leqslant 2 \lambda_n)
\nonumber\\&=P(X_n\leqslant 2\lambda_n)\big(1-\prod_{z\in\Sigma}\big(1-P(A(\Delta_n,\zeta_z^{N'_z})\not\subset\mathbb{B}(0,n+3h(n)))\big)\big).
\end{align}
Or, d'après la proposition \ref{step2}\footnote{ce résultat ne découle pas directement de la proposition \ref{step2} à cause de la probabilité conditionnelle, mais la démonstration est exactement la même.},
\begin{align}
\label{18}
\nonumber &P(A(\Delta_n,\zeta_z^{N'_z})\not\subset\mathbb{B}(0,n+3h(n)))
\nonumber\\&\geqslant P(A(\Delta_n,\zeta_z^{N'_z})\not\subset\mathbb{B}(0,n+3h(n))\ \normalfont{|}\ N'_z\geqslant c(3h(n)+L(n)))P(N'_z\geqslant c(3h(n)+L(n)))
...
\begin{Lemme}
 ...
 \begin{proof}
  ...
  \end{equation*}
 De plus, d'après le lemme \ref{b(n)}, $c_4h(n)n^{d-1}\leqslant\lambda_n\leqslant c_5h(n)n^{d-1}$, donc \newline$c_6 h(n) \leqslant\mu\leqslant c_7 h(n)$. 
 
 On a donc, en posant $a=\lfloor c_1 h(n) \rfloor +1$ :
 \begin{align*}
...
\begin{align}
 \nonumber&P(\exists k\geqslant n,\ \delta_0(k)\geqslant h(k)\normalfont\ {|}\ \delta_I(n)<L(n))
 \nonumber\\\geqslant& P(X_n\leqslant 2\lambda_n)\big(1-\prod_{z\in\Sigma}\big(1-\exp(-c(4h(n))^2-Ch(n))\big)\big)
 \nonumber\\\geqslant& P(X_n\leqslant 2\lambda_n)(1-\exp(-\kappa n^{d-1}\exp(-Ch(n)-c'h(n)^2)))
 \nonumber\\\geqslant& P(X_n\leqslant 2\lambda_n)(1-\exp(-\kappa n^{d-1}\exp(-c''h(n)^2)))
 \nonumber\\=&P(X_n\leqslant 2\lambda_n)(1-\exp(-\kappa n^{d-1-\alpha^2c''})).
\end{align}
...
Il n'est pas clair \emph{a priori} que ce théorème nous donne vraiment une indication sur l'optimalité de $h(n)$ pour l'erreur externe, mais on peut voir qu'il implique le résultat intéressant suivant (Corollaire \ref{cio}) :
...
Autrement dit, pour une infinité de termes, l'erreur interne ou l'erreur externe est d'ordre $\sqrt{\log n}$, donc le théorème \ref{limsup} est d'une certaine façon optimal. On verra dans la partie \ref{interne} que (\ref{io2}) est en fait vraie pour l'erreur interne seule.
...
\end{Def}
 \begin{figure}[H]
 \centering
   \includegraphics{Ik.jpg}
   \caption{Exemple de réalisation de $I_k$}
   \label{ik}
 \end{figure}
...
\begin{proof}
 ...
 D'autre part,
  ...
  \begin{eqnarray}
   ...
   \nonumber\\ & \geqslant & h(R_{k}) \ \ \mbox{par (\ref{r1}), pour $n$ assez grand,}
   \nonumber
  \end{eqnarray}
  ce qui prouve $(i)$.
  ...
  \begin{eqnarray*}
   ...
   \nonumber\\ & \geqslant & L(R_{k}) \ \ \mbox{par (\ref{r1}), pour $n$ assez grand.}
   \nonumber
  \end{eqnarray*}
...
\begin{itemize}
 \medskip
 \item Pour la première, on se sert du fait que $P_0 (S(H(\Sigma (z))) = y) \leqslant \frac{c_1} {\| z \| ^ {d-1}} $ (Lemme 1.7.4 de \cite{ref3}), qui vient du fait que la probabilité est à peu près uniforme sur $\Sigma(z)$, et que $P_y (H(z) < \infty ) \leqslant \frac{c_2}{1+\| y-z \| ^{d-2}}$ (Théorème 1.5.4 de \cite{ref3}), ainsi que de la propriété de Markov fort, pour écrire :
 \begin{eqnarray*}
 E[N_z (X, \Lambda, \infty )] & = & \lambda \ P_0 (S(H(\Sigma (z))) \in \Lambda, \ H(z) < \infty)
 \\& = & \lambda \ P_0 (S(H(\Sigma (z))) = z)
 \\ & & + \lambda \sum _{y \in \Lambda \setminus \{z\} } P_0 (S(H(\Sigma (z))) = y, \ H(z) < \infty )
 \\& = & \lambda \ P_0 (S(H(\Sigma (z))) = z)
 \\ & & + \lambda \sum _{y \in \Lambda \setminus \{z\} } P_0 (S(H(\Sigma (z))) = y) \ P_y (H(z) < \infty )
\\& \leqslant & \lambda \frac{c_1} {\| z \| ^ {d-1}} \left( 1 + \sum _{y \in \Lambda \setminus \{z\} } \frac{c_2}{1+\| y-z \| ^{d-2}} \right). \nonumber
 \end{eqnarray*}
 \pagebreak[1]
 ...
 \\& \leqslant & c_5 \frac{\lambda R} {\| z \| ^ {d-1}}. \nonumber
 \end{eqnarray*}
 ...
 \\& = & \lambda \sum _{y \in \Lambda '} P_0 (S(H(\Sigma (z))) = y) \ P_y (H(z) < H( \mathbb{B}^c (0, \| z \| + h )) )
 \\& \leqslant & \lambda \sum _{y \in \Lambda '} \frac{c_1} {\| z \| ^ {d-1}} \ \frac{c_6 R^2}{\| z-y \| ^d}
 \\& \leqslant & c_7 \frac{\lambda R^2} {\| z \| ^ {d-1}} \sum _{k=R} ^{2\|z\|+2} \frac{k^{d-2}}{k^{d}}
 \\& \leqslant & c_8 \frac{\lambda R} {\| z \| ^ {d-1}}, \nonumber
 \end{eqnarray*}
...
\left( - \kappa h(R_{F(n)}) L(R_{F(n)}) \right)$. Notre but est de montrer que $\lim_{n \to \infty } P \left( \bigcup _{k \leqslant F(n)} A_k \right) = 1$.
...
\begin{proof}
Par récurrence :
si la propriété est vraie pour $N < F(n)$ alors :
\begin{eqnarray}
...
\begin{figure}[H]
 \centering
   \includegraphics[scale=0.65]{cacahuete.jpg}
   \caption{Région obtenue pour deux sources}
\end{figure}
Dans cette simulation, on a choisi de séparer les deux sources de $4/5$ fois le rayon théorique $\rho (N)$, et de lancer $N$ explorateurs depuis chaque source.

Ici, la propriété abélienne du paragraphe \ref{abel} est importante. Elle permet d'affirmer que la loi finale de la région ne dépend pas de l'ordre de lancer des explorateurs.

\subsection{Décalage vers la droite}
Choisissons de faire voyager nos $N$ explorateurs non plus selon des marches aléatoires symétriques, mais selon des marches déséquilibrées vers un côté. Par exemple, sur $\mathbb{Z}^2$, construisons une marche en suivant les probabilités
\[P(\mathrm{haut}) = 1/4,\  P(\mathrm{bas}) = 1/4, \  P(\mathrm{gauche}) = 1/4 - \epsilon, \ P(\mathrm{droite}) = 1/4 + \epsilon . \]

\begin{figure}[H]
 \centering
   \includegraphics[scale=0.65]{drift.jpg}
...
\begin{figure}[H]
 \centering
   \includegraphics[scale=0.3]{log.jpg}
   \caption{Log de la distance maximale à l'origine (en noir) et pente 2/3 (en bleu)}
\end{figure}

Ceci ne semble pas étonnant, à cause de l'argument suivant : une telle marche déséquilibrée a tendance, en projection sur l'horizontale, à s'écarter vers la droite linéairement avec le temps $t$. En projection verticale, elle dévie comme $\sqrt{t}$ (marche symétrique unidimensionnelle). On s'attend à ce que la région obtenue respecte ces ordres de grandeur, donc son étendue horizontale devrait varier comme le carré de son étendue verticale. Le produit des deux vaut $N$, donc on s'attend à $N^{2/3}$ horizontalement et $N^{1/3}$ verticalement.

\subsection{Cavaliers explorateurs}
On peut aussi choisir de faire des marches aléatoires différentes, en s'autorisant plus de pas. Il pourrait être intéressant de se placer dans un cadre général en considérant des matrice de transition quelconqes pour la marche aléatoire.

Par exemple, on peut explorer en se déplaçant comme un cavalier aux échecs, aléatoirement. La région explorée est assez semblable à celle du cas du IDLA, l'avantage principal étant qu'elle est plus rapide à simuler car les cavaliers aléatoires s'éloignent plus vite de l'origine que les marcheurs aléatoires.
\begin{figure}[H]
 \centering
   \includegraphics[scale=0.21]{cavaliers.jpg}
   \caption{Région explorée par des cavaliers ($300000$ cavaliers)}
\end{figure}
...
\label{b(n)}
\begin{enumerate}[i)]
\item Soit $\omega_d$ le volume de la boule unité de $\mathbb{R}^d$, on a :
 \begin{equation*}
  b(n)\sim\omega_dn^d .
 \end{equation*}
...
Et on a donc :
\begin{equation*}
 | \frac{b(n)}{V(n)}-1 | \leqslant\frac{1}{V(n)}(\normalfont{|}c(n)-b(n)\normalfont{|}+\normalfont{|}c(n)-V(n)\normalfont{|})=O(\frac{1}{n}) .
\end{equation*}
