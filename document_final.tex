...
On suppose dans cette partie que l'évènement $A^c_{k-1}$ est réalisé, pour $1 \leqslant k \leqslant F(n)-1$. Utilisons alors l'ensemble non vide donné par \ref{nonvide} au rang $k-1$ : il existe un $Z_k$ non encore visité, tel que $\|Z_k\| \leqslant R_{k-1} + 1$. Comme $A^c_{k-1}$ est réalisé, nécessairement, 
\begin{equation}
...
\begin{eqnarray}
 ...
 \nonumber\\& \geq & \exp \left( - \kappa h(n) L(n) \right)
\end{eqnarray}
...
\left( - \kappa h(n) L(n) \right)$. Notre but est de montrer que $\lim _{n \to \infty } P \left( \bigcup _{k \leqslant F(n)} A_k \right) = 1$.
