...
\begin{Rq}
 ...
 \end{equation}
 En effet, le cas échéant, on aurait $(\mathbb{B} (0, R_k)\cup\partial\mathbb{B} (0, R_k)) \subset A(b(R_k))$ et donc strictement plus de $b(R_k)$ sites seraient occupés, alors qu'on n'a lancé que $b(R_k)$ explorateurs.
\end{Rq}
...
\end{equation}
 ...
 \begin{equation}
  I_k = \{ N_{Z_k} (X_k, \Lambda_k, \infty ) = 0, N_{Z_k} (X_k, \Lambda '_k, 7 L(R_{k-1})) = 0 \}
 \end{equation}
\end{enumerate}
\end{Def}
...
Cela sera assuré pour $X_k$ dans un bon intervalle, donné par le lemme suivant, qui induira du coup le choix des paramètres $\lambda_k$ :
\begin{Lemme}
...
\begin{proof}
 D'une part,
  \begin{eqnarray}
   X_k\leqslant 2(b(R_{k-1} + 2 h(R_{k-1})) - b(R_{k-1})) & \Rightarrow & X_k + b(R_{k-1}) \leqslant b(R_{k-1} + 4 h(R_{k-1})) - b(R_{k-1})
   \nonumber\\&\Rightarrow & R_{k}\leqslant R_{k-1}+4h(R_{k-1}) \label{r1}
  \end{eqnarray}
 D'autre part,
  \begin{eqnarray}
   X_k\geqslant \frac23 (b(R_{k-1} + 2 h(R_{k-1})) - b(R_{k-1})) & \Rightarrow & X_k + b(R_{k-1}) \geqslant b(R_{k-1} + \frac43 h(R_{k-1})) + \frac13 b(R_{k-1})
...
Résumons ce que l'on a fait jusqu'à présent : on a montré que lorsque $A^c_{k-1}$ est réalisé, il existe un évènement $I_k$ tel que $\frac{2}{3} \lambda_k \leqslant X_k \leqslant 2 \lambda_k \Rightarrow I_k \subset A_k \cup B_k$ et $P(I_k) \geqslant \exp 
...
On note $C_k$ l'évènement $\{ \frac{2}{3} \lambda_k \leqslant X_k \leqslant 2 \lambda_k \}$. On note $\mathcal{G}_k$ la filtration associée à l'exploration par vagues.
\begin{Lemme} \label{indic}
 \[ 1_{A_{k-1}^c} P(A_k ^c  \cap C_k \mid \mathcal{G}_{k-1}) \leqslant P(B_k \mid \mathcal{G}_{k-1} ) + 1_{A_{k-1}^c} (1-P(I_k \mid \mathcal{G}_{k-1}) ) \]
\end{Lemme}
\begin{proof}
La propriété essentielle $C_k \cap I_k \subset A_k \cup B_k $ donne :
\begin{eqnarray}
 P(I_k \cap C_k \mid \mathcal{G}_{k-1}) & \leqslant &  P((A_k \cap C_k) \cup B_k \mid \mathcal{G}_{k-1})
 \nonumber\\& \leqslant & P(A_k \cap C_k \mid \mathcal{G}_{k-1}) + P(B_k \mid \mathcal{G}_{k-1})
 \label{croustille}
\end{eqnarray}
D'où, en utilisant que $C_k$ est indépendant de $\mathcal{G}_{k-1}$ puis l'inégalité \ref{croustille} : 
\begin{eqnarray}
 1_{A_{k-1}^c} P(A_k ^c  \cap C_k ) & = & 1_{A_{k-1}^c}(P(C_k) - P(A_k \cap C_k \mid \mathcal{G}_{k-1}))
 \nonumber \\ & \leqslant & 1_{A_{k-1}^c} \left( P(C_k) - P(I_k \cap C_k \mid \mathcal{G}_{k-1}) + P(B_k \mid \mathcal{G}_{k-1}) \right)
\end{eqnarray}
Pour conclure, il suffit de montrer que $P(C_k) - P(I_k \cap C_k \mid \mathcal{G}_{k-1}) \leqslant 1 - P(I_k \mid \mathcal{G}_{k-1})$, ce qui est facile en remarquant que $C_k^c \cup (I_k \cap C_k) \supset I_k$.
\end{proof}

On montre maintenant par récurrence sur $N \in \mathbb{N}$ la propriété :
...
Cette propriété est vraie en $N=0$, et si elle est vraie pour $N < F(n)$ alors :
\begin{eqnarray}
 P\left( \bigcup_{k \leqslant N+1} A_k^c \cap C_k \right) & \leqslant & E[1_{\bigcup_{k \leqslant N} A_k^c \cap C_k} P(A_N^c \cap C_N \mid \mathcal{G}_{N-1} ) ]
 \nonumber \\ & \leqslant & E[1_{\bigcup_{k \leqslant N} A_k^c \cap C_k} \left(  P(B_N \mid \mathcal{G}_{N-1} ) + 1-P(I_N \mid \mathcal{G}_{N-1}) \right) ]
 \nonumber
\end{eqnarray}
en utilisant le lemme \ref{indic}. On en déduit :
\begin{eqnarray}
 P\left( \bigcup_{k \leqslant N+1} A_k^c \cap C_k \right) & \leqslant & P(B_N) + (1-\exp (\alpha (n) ) ) P\left( \bigcup_{k \leqslant N} A_k^c \cap C_k \right)
 \nonumber \\ & \leqslant & P(B_N) + (1-\exp (\alpha (n) ) ) \left( \sum _{k \leqslant N} P(B_k) - \left( 1 - \exp (\alpha (n) ) \right) ^N \right)
 \nonumber \\ & \leqslant & \sum _{k \leqslant N+1} P(B_k) - \left( 1 - \exp (\alpha (n) ) \right) ^{N+1}
 \nonumber
\end{eqnarray}
ce qui achève la preuve de \ref{rec}.
...
\end{equation}
et en appliquant \ref{cups} puis \ref{rec} avec $N=F(n)$, on trouve :
\begin{equation} \label{fin}
 P\left( \bigcup_{k \leqslant F(n)} A_k \right) \geqslant 1 - \sum_{k \geqslant F(n)} \left( P(B_k) + P(C_k^c) \right) - (1 - \exp ( - \kappa h(R_{F(n)}) L(R_{F(n)}) ) ) ^{F(n)}
\end{equation}

On va alors choisir $F(n) = \lfloor \frac{n}{h(n)} \rfloor$, en effet, pour ce choix on a :
\begin{itemize}
 \item $P(B_k)$ décroît plus vite que toute puissance de $n$ d'après un résultat de \cite{ref} donc $\sum_{k \geqslant F(n)} P(B_k) \to_{n \to \infty} 0$.
 ...
 \begin{equation*}
 \sum_{k \geqslant F(n)} P(C_k^c) \leqslant \frac{n}{h(n)} \frac{C}{h(n)n^{d-1}} \underset{n\to+\infty}{\longrightarrow} 0
 \end{equation*}
 \item $R_{F(n)}$ est d'ordre $n$ donc
  \begin{equation*}
  (1 - \exp ( - \kappa h(R_{F(n)}) L(R_{F(n)}) ) ) ^{F(n)} \leqslant \exp\left(-\frac{n}{h(n)} \exp (- \kappa ' h(n) L(n) ) \right)
  \end{equation*}
  Or, si $h(n) = L(n) = \sqrt{\alpha \log (n) }$,
  \begin{equation*}
  \frac{n}{h(n)} \exp (- \kappa ' h(n) L(n) ) = \frac{n^{1-\kappa ' \alpha}}{\sqrt{\alpha \log (n) }} \underset{n\to+\infty}{\longrightarrow} +\infty
  \end{equation*}
  pourvu qu'on ait pris $\alpha$ assez petit ($\alpha < 1 / \kappa'$).
\end{itemize}
...
\begin{proof}
 ...
 \begin{equation*}
  P(X_\lambda< 2 \lambda) \leqslant P(|X_\lambda - E[X_\lambda] | \leqslant \lambda)
 \end{equation*}
