...
\begin{Def}
 ...
 \newline On notera par la suite $A(\xi^n)=A(\emptyset,\xi^n)$ et $A_R(\xi^n)=A(\xi^n)\cap\mathbb{B}(0,R)$. On notera aussi plus simplement $A(n)=A( (0,\dots,0) )$ où le vecteur est de taille $n$.
 \end{Def}
...
\ et\ \limsup\limits_{n\to\infty}\frac{\delta_O(n)}{\log(n)}\leqslant\beta_2  
 ...
 \begin{The}
 \label{ext}
  En dimension d$\geqslant3$, $\exists\alpha_d$>0 tel que
  \begin{equation}
...
\end{Cor}
Autrement dit, pour une infinité de termes, l'erreur interne ou l'erreur externe est d'ordre $\sqrt{\log n}$, donc le théorème \ref{limsup} est d'une certaine façon optimal. On verra dans la partie \ref{interne} que \ref{io} est en fait vraie pour l'erreur interne seule, mais on ne sait pas si elle est vraie pour l'erreur externe seule.
...
\label{interne}
Pour $z \in \mathbb{Z}^d \setminus \{ 0 \}$ on note $\Sigma (z) = \partial \mathbb{B} (0, \| z \| )$. Remarquons que $z \in \Sigma (z)$. Pour $\Lambda \subset \Sigma (z)$ et $n$ un entier et $R > 0$, on définit aussi la variable aléatoire $N_z (n, \Lambda, R)$ comme le nombre de marches aléatoires parmi $n$ marches partant de $0$ qui atteignent $\Sigma (z)$ en $\Lambda$ puis visitent $z$ avant de sortir de $\mathbb{B} (0, \| z \| + R)$.
\begin{Lemme}
