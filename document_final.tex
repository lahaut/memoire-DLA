...
\begin{enumerate}[i)]
 \item
 Soit $n$ un entier et $R > 0$, on définit la variable aléatoire $N_z (n, R)$ comme le nombre de marches aléatoires parmi $n$ marches partant de $0$ visitent $z$ avant de sortir de $\mathbb{B} (0, \| z \| + R)$ :
\begin{equation}
...
\begin{proof}
 ...
 c'est-à-dire le nombre de marches parmi les $N_z (n, R)$ qui entrent en $\Sigma (z)$ par $\Lambda$.
...
Résumons ce que l'on a fait jusqu'à présent : on a montré qu'il existait un évènement $I_k$ tel que $\frac{2}{3} \lambda_k \leqslant X_k \leqslant 2 \lambda_k \Rightarrow A_{k-1}^c \cap I_k \subset A_k \cup B_k$ et $P(I_k) \geqslant \exp 
...
\begin{eqnarray}
 ...
 \nonumber \\ & \leqslant & P(B_{N+1}) + (1-\exp (\alpha (n) ) ) \left( \sum _{k \leqslant N} P(B_k) - \left( 1 - \exp (\alpha (n) ) \right) ^N \right)
