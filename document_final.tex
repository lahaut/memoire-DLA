...
On constate que \{$\delta_O(R_n)\geqslant h(R_n)$\} est lié à $\{\mathrm{cov}(z), z\in\Sigma\}$ de la façon suivante :
\begin{align}
...
\end{figure}
Dans cette illustration, on a choisi de séparer les deux sources de $4/5$ fois le rayon théorique $\rho (N)$, et de lancer $N$ explorateurs depuis chaque source.

Il est ici important de noter que la propriété abélienne énoncée en \ref{abel} permet d'affirmer que la loi finale de la région ne dépend pas de l'ordre de lancer des explorateurs.
...
On peut aussi choisir de faire des marches aléatoires différentes, en s'autorisant plus de pas. Il pourrait être intéressant de se placer dans un cadre plus général de matrice de transition pour la marche aléatoire.
