...
\begin{Lemme}
\label{b(n)}
i)Soit $\omega_d$ le volume de la boule unité de $\mathbb{Z}^d$, on a :
 \begin{equation*}
  b(n)\sim\omega_dn^d
 \end{equation*}
ii)Soit $a_n$ une suite telle que $a_n\to\infty$ et $a_n=o(n)$, on a :
 \begin{equation*}
   b(n+a_n)-b(n)\sim\omega_da_nn^{d-1}
 \end{equation*}
 \begin{proof}
 i) Notons V(n)=$\omega_dn^d$ le volume de la boule de rayon n et $\alpha_d=\sqrt d$ le diamètre d'un cube unité. On peut paver $\mathbb{R}^d$ par des cubes unités dont les centres z appartiennent à $\mathbb{Z}^d$, notés $c_z$. On pose alors :
 \begin{equation*}
  A(n)=\bigcup_{z, c_z\cap B(0,n)\neq\emptyset}{c_z}
 \end{equation*}
On remarque que c(n):=$\lambda$(A(n))=Card($\{z, c_z\cap B(0,n)\neq\emptyset\}$)
\begin{align*}
 \normalfont{|}c(n)-V(n)\normalfont{|}=c(n)-V(n)&\leqslant\lambda(\bigcup_{z, c_z\cap B(0,n)\neq\emptyset,c_z\not\subset B(0,n)}{c_z})
...
De même :
\begin{align*}
 \normalfont{|}c(n)-b(n)\normalfont{|}=c(n)-b(n)&\leqslant Card(\{z, c_z\cap B(0,n)\neq\emptyset,c_z\not\subset B(0,n)\})
 \\&\sim2\alpha_dn^{d-1}\omega_d
\end{align*}
Et on a donc :
\begin{equation*}
...
ii) Posons :
 \begin{equation*}
  A'(n)=\bigcup_{z, c_z\subset B(0,n)}{c_z}
 \end{equation*}
 et c'(n)=$\lambda$(A'(n))
 \begin{align*}
  &b(n+a_n)-b(n)
